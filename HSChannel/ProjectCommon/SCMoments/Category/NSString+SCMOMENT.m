...
#import "NSString+SCMOMENT.h"

@implementation NSString (SCMOMENT)
...
{
    return [self scm_sizeWithFont:font maxW:MAXFLOAT];
}
...
+ (NSString *)scm_timeWithDate:(NSDate *)date
{
    ...
    else if(interval >= 24*60*60*1+nowHour*60*60+nowMinute*60+nowSecond)//大于等于2天
    {
        ...
        {
            result = [NSString stringWithFormat:@"%@ %02zi:%02zi", [self scm_stringedWeekday:dateWeekday], dateHour, dateMinute];
        }
    }
...
+ (NSString *)scm_stringedWeekday:(NSUInteger)weekday
{
