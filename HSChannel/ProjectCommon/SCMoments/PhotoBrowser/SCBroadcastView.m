...
@interface SCBroadcastView()<UIScrollViewDelegate>
...
@implementation SCBroadcastView
...
- (void)layoutSubviews
{
    ...
    for (SCBroadcastViewCell *cell in self.visibleCells) {
...
{
    SCBroadcastViewCell *cell = nil;
    for (SCBroadcastViewCell *aCell in self.reusableCells) {
...
- (void)showCells
{
    ...
    if (self.reusableCells.count>2) {
        ...
        for (SCBroadcastViewCell *cell in self.reusableCells) {
...
- (void)showCellAtIndex:(NSUInteger)index
{
    ...
    SCBroadcastViewCell *cell = [self.dataSource broadcastView:self cellAtPageIndex:cellIndex];
