...
@implementation SCMomentsListViewModel
...
- (void)loadStatusWithCount:(NSInteger)count Completed:(void (^)(BOOL isSuccessed))completed
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        SCMomentsModel *model = [[SCMomentsModel alloc] init];
...
//        model.msgContent = @"";
        ...
        for (int i = 0; i < likeItemsRandom; i++) {
            SCMomentsCellLikeItemModel *likeModel = [[SCMomentsCellLikeItemModel alloc] init];
        ...
        SCMomentsViewModel *momentsViewModel = [SCMomentsViewModel viewModelWithStatus:model];
...
{
    SCMomentsViewModel *model = self.statusList[indexPath.row];
...
//    NSString *name = [[EMClient sharedClient] currentUsername];
    ...
    if (!model.isLiked) {
        SCMomentsCellLikeItemModel *likeModel = [SCMomentsCellLikeItemModel new];
        likeModel.userName = name;
        likeModel.userId = name;
        [temp addObject:likeModel];
        model.liked = YES;
    } else {
        SCMomentsCellLikeItemModel *tempLikeModel = nil;
        for (SCMomentsCellLikeItemModel *likeModel in model.likeItemsArray) {
