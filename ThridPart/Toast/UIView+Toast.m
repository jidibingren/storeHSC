...
static const CGFloat CSToastActivityOpacity     = 0.7;
...
static const BOOL CSToastActivityShowCloseButton             = NO;
...
- (void)makeToastActivity:(id)position {
    ...
    activityView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:CSToastActivityOpacity];
    ...
    if (CSToastActivityShowCloseButton) {
        UIButton* cancelButton = [[UIButton alloc]init];
        [cancelButton setImage:[UIImage imageNamed:@"btn_cancel_spinner"] forState:UIControlStateNormal];
        [cancelButton sizeToFit];
        cancelButton.bounds = CGRectMake(0, 0, cancelButton.bounds.size.width/2, cancelButton.bounds.size.height/2);
        cancelButton.center = CGPointMake(CSToastActivityWidth - cancelButton.bounds.size.width/2 - 5,
                                          cancelButton.bounds.size.height/2 + 5);
        [activityView addSubview:cancelButton];
        [cancelButton addTarget:self action:@selector(hideToastActivity) forControlEvents:UIControlEventTouchUpInside];
...
- (void)hideToastActivity {
    [self hideToastActivity:YES];
}

static char associatedToastActivityHideCallback;

- (void)hideToastActivity: (BOOL)animated {
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
    if (existingActivityView != nil) {
        objc_setAssociatedObject (self, &CSToastActivityViewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        ToastActivityHideCallback callback = objc_getAssociatedObject(self, &associatedToastActivityHideCallback);
        if (callback) {
            callback();
            objc_setAssociatedObject(self, &associatedToastActivityHideCallback, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        if (!animated) {
...
- (void) makeToastActivityWithHideCallback: (ToastActivityHideCallback)callback
{
    objc_setAssociatedObject(self, &associatedToastActivityHideCallback, callback, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if(image != nil) {
        ...
        imageWidth = MIN(imageWidth, CSToastImageViewWidth);
        imageHeight = MIN(imageHeight, CSToastImageViewHeight);
        imageView = [[UIImageView alloc] initWithImage:image];
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        imageView.frame = CGRectMake(CSToastHorizontalPadding, CSToastVerticalPadding, imageWidth, imageHeight);
    }
    ...
    // Make title, message and image at the center of this view
    CGFloat verticalSpaceHeight = wrapperHeight - (messageTop + messageHeight);
    if (titleLabel != nil) {
        titleTop = (wrapperHeight - verticalSpaceHeight) / 2;
    }
    if (messageLabel != nil) {
        messageTop = titleTop + titleHeight + CSToastVerticalPadding;
    }
    if (imageView != nil) {
        ...
        imageFrame.origin.y = (wrapperHeight - imageFrame.size.height) / 2;
